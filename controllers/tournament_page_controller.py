...
class TournamentPageController:
    def __init__(
        self, model: Tournament, view: TournamentPageView, main_controller
    ) -> None:
        self._model = model
        self._view = view
    ...
    def _connect_match_areas(self) -> None:
        match_result_areas = [
            item
            for item in self._view.graphics_view.scene().items()
    ...
    def _update_match_result(
        ...
        if match.score_participant1 != score1 or match.score_participant2 != score2:
            self._model.brackets[0].update_result(
                match_stage, match_number, (score1, score2)
            )
            self._view.redraw()
            self._connect_match_areas()
